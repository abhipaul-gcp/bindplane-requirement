...
# THIS VERSION IS FOR ROOT USER (no sudo commands)
#
...
echo "=== PostgreSQL 16 Installation for BindPlane (Offline Mode - Root) ==="
...
if ! id postgres &>/dev/null; then
  ...
  useradd -r -m -d /var/lib/pgsql -s /bin/bash -c "PostgreSQL Server" postgres
...
if grep -q "BindPlane Production Configuration" $PGDATA/postgresql.conf 2>/dev/null; then
  ...
  if [ "$RECONFIG" = "yes" ]; then
    ...
    if [ -f "$PGDATA/postgresql.conf.backup."* ]; then
      ...
      cp $LATEST_BACKUP $PGDATA/postgresql.conf
...
cp $PGDATA/postgresql.conf $PGDATA/postgresql.conf.backup.$(date +%Y%m%d_%H%M%S)

# Apply production-grade PostgreSQL settings for 2 TB/day workload
tee -a $PGDATA/postgresql.conf > /dev/null <<'EOF'
...
if [ ! -f /usr/lib/systemd/system/postgresql-16.service ]; then
  ...
  tee /usr/lib/systemd/system/postgresql-16.service > /dev/null <<'SERVICEEOF'
[Unit]
...
tee /usr/lib/tmpfiles.d/postgresql-16.conf > /dev/null <<'TMPFILESEOF'
...
if ! systemctl is-active --quiet postgresql-16; then
  ...
  echo "Please check the service status: systemctl status postgresql-16"
  echo "Check logs: journalctl -xeu postgresql-16.service"
  exit 1
fi
...
echo ""
if su - postgres -c "psql -lqt" | cut -d \| -f 1 | grep -qw bindplane; then
  ...
  if [ "$RECREATE_DB" = "yes" ]; then
    ...
    su - postgres -c "psql -c 'DROP DATABASE IF EXISTS bindplane;'" 2>/dev/null || true
    su - postgres -c "psql -c 'DROP USER IF EXISTS bindplane;'" 2>/dev/null || true
...
if grep -q "^host.*bindplane.*bindplane" $PGDATA/pg_hba.conf; then
  ...
  tee -a $PGDATA/pg_hba.conf > /dev/null <<'EOF'
...
echo ""
systemctl status postgresql-16 --no-pager | head -15
...
echo ""
echo "Next Step:"
echo "  Run: bash install-linux.sh -f /tmp/bindplane-packages/management/bindplane-ee_linux_amd64.rpm --init"
echo ""
